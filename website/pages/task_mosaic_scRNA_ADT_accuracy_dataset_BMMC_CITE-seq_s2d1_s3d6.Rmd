---
params:
  task: mosaic_scRNA+ADT
  dataset: BMMC_CITE-seq_s2d1_s3d6
  fig_dir: false
...
## prepare palette
library(ggsci)
methods_pallette <- pal_d3(palette = "category20", alpha = 1)(length(unique(metrics$method)))
# library(scales)
# show_col(methods_pallette)
names(methods_pallette) <- unique(metrics$method)

metrics_type = "batch effect removal"
...
Batch effect removal (CITE+RNA) {.tabset}
--------------------

```{r metrics-batch-NoADT, results = "hide"}
metrics_path = "Acc_pired_NoADT_path"
metrics <- read.csv(here("data", dataset_info[, metrics_path]), check.names = FALSE)
...
Biological conservation (CITE+RNA) {.tabset}
-----------------------

```{r metrics-bio-NoADT, results = "hide"}
metrics_path = "Acc_pired_NoADT_path"
metrics <- read.csv(here("data", dataset_info[, metrics_path]), check.names = FALSE)
...
Cell alignment accuracy {.tabset}
-------------------

```{r metrics-align, results = "hide"}
metrics_path = "Acc_unpired_path"
metrics <- read.csv(here("data", dataset_info[, metrics_path]), check.names = FALSE)
...
Imputation accuracy {.tabset}
-------------------

```{r metrics-imp, results = "hide"}
metrics_path = "Imp_path"
metrics <- read.csv(here("data", dataset_info[, metrics_path]), check.names = FALSE)

metrics_type = "imputation accuracy"
metrics_name <- intersect(colnames(metrics), metrics_info[metrics_info$Group == metrics_type, "Metrics"])
