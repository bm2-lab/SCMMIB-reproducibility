...
# Batch effect removal {.tabset .tabset-dropdown .tabset-fade}

```{r metrics-batch, results = "hide"}
metrics_path = "Rob_paired_path"
...
full_src <- make_mosaic_method_robustness_md(metrics, metrics_name, "s2d1", "s3d6")
...
# Batch effect removal (CITE+RNA) {.tabset .tabset-dropdown .tabset-fade}

```{r metrics-batch-NoADT, results = "hide"}
metrics_path = "Rob_paired_NoADT_path"
metrics <- read.csv(here("data", dataset_info[, metrics_path]), check.names = FALSE)

metrics_type = "batch effect removal"
metrics_name <- intersect(colnames(metrics), metrics_info[metrics_info$Group == metrics_type, "Metrics"])

full_src <- make_mosaic_NoADT_method_robustness_md(metrics, metrics_name, "s2d1", "s3d6")
out <- knitr::knit_child(text = unlist(full_src), options = list(cache = FALSE))
```
...
# Biological conservation (CITE+RNA) {.tabset .tabset-dropdown .tabset-fade}
